...
class ProductAppTests(unittest.TestCase):
    ...
    def test_product_ui_uses_automatic_identity_and_confirmed_defaults(self):
        ...
        self.assertIn('id="account-migration-card"', template)
        self.assertIn("导出当前账号迁移包", template)
        self.assertIn("/api/accounts/import-bundle", template)
        ...
        self.assertIn('id="review-stop-after-empty-pages"', template)
        self.assertIn("连续 3 页无新增时提前停止", template)
        self.assertIn("B站已关闭/不可回复", template)
